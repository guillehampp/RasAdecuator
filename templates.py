...
class TemplateHandler:
    ...
    def render_ssp_offline_fast(self,template_name,output_name,att_product,precision_product):
        ...
        with open(output_name, 'a') as f:  # Deja 'a' para agregar al final del archivo
            f.write(output)
    def render_offline_fast_final(self,template_name,output_name,att_product,precision_product):
        parameters = []
        outputs = []
        parameters.append({'name': 'Precision Attitude Product', 'type': 'XPNetStringNotTimeTagged', 'value': att_product})
        output = self.render_template(template_name,  parameters)
    ...
    def render_arg2(self,template_name,output_name,att_product,precision_product):
        ...
        with open(output_name, 'a') as f:  # Deja 'a' para agregar al final del archivo
            f.write(output)
    def render_arg3(self,template_name,output_name,att_product,precision_product,total_electron_content):
        parameters = []
        outputs = []
        parameters.append({'name': 'Precision Attitude Product', 'type': 'XPNetStringNotTimeTagged', 'value': att_product})
        output = self.render_template(template_name,  parameters)
        ...
        parameters.append({'name': 'Total Electron Content CUSS Product', 'type': 'XPNetStringNotTimeTagged', 'value': total_electron_content})
        output = self.render_template(template_name,  parameters)
        outputs.append(output)
        with open(output_name, 'a') as f:  # Deja 'a' para agregar al final del archivo
            f.write(output)